...
class DB_MySQL  {
	...
	function setcharset($charset) {
		if ($charset && $this->charsetdb[$charset]) {
			if(function_exists('mysql_set_charset')) {
				mysql_set_charset($this->link, $this->charsetdb[$charset]);
			} else {
	...
	function fetch($query, $result_type = MYSQLI_ASSOC) { //MYSQL_NUM
	...
	function sqldump($table, $fp=0) {
		...
		while ($row = $this->fetch($rows, MYSQLI_NUM)) {
